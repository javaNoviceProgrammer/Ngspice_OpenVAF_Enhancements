...
# --- (re)compile the models for THIS platform (.osdi is arch-specific) ---
...
_compile() {
  local src="$1" out="$2"
  if ! ( cd "$_SETUP_DIR" && "$VAF" "$src" -o "$out" ) >/dev/null 2>&1; then
    echo "openvaf-r failed to compile $src" >&2
    exit 1
  fi
  [ -f "$out" ] || { echo "$src was not compiled to $out" >&2; exit 1; }
}
