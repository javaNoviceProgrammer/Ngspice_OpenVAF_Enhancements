...
# cross-check (laplace_variants), the isolated laplace_zd fixture, and the
# array-variable/array-literal mixed-argument fixture, and write dc.txt,
# ac.txt, tran.txt, dc_variants.txt, dc_zd_only.txt, dc_mixed.txt. Works on
...
. "$DIR/_setup.sh"        # sets NG, VAF, OSDI, OSDI_VARIANTS, OSDI_ZD, OSDI_MIXED for this platform
cd "$DIR"
...
base="dc_mixed.cir"
out="${DIR}/dc_mixed.txt"
work="${DIR}/.dc_mixed.cir"
sed -e "s|RESULTFILE|${out}|" -e "s|OSDIFILE_MIXED|${OSDI_MIXED}|" "$base" > "$work"
"$NG" --batch "$work" >"${DIR}/dc_mixed.log" 2>&1
rm -f "$work"
echo "[dc_mixed] wrote $out"
