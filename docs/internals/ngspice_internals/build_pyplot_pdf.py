#!/usr/bin/env python3
"""Regenerate the PDF edition of the pyplot command reference.

Usage:  python3 docs/internals/ngspice_internals/build_pyplot_pdf.py
Needs pandoc + xelatex (and STIX Two Text / Menlo, as the sibling notes do).
Repo-relative links become GitHub URLs so they stay clickable in the standalone PDF.
"""
import datetime
import os
import re
...
def strip_front_matter(text):
    """The .md carries a YAML title block; pandoc gets title/subtitle via -V instead."""
    if text.startswith("---"):
...
def githubify(text):
    def rewrite(m):
        ...
        if target.startswith(("http://", "https://", "#")):
            return m.group(0)
        rel = os.path.relpath(
            os.path.normpath(os.path.join(ROOT, target.split("#")[0])), ROOT)
...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        src = strip_front_matter(open(os.path.join(HERE, STEM + ".md")).read())
        open(md, "w").write(githubify(src))
        ...
        r = subprocess.run([
            ...
            "-V", "title=The pyplot command",
            "-V", "subtitle=Matplotlib plotting in ngspice — complete reference",
