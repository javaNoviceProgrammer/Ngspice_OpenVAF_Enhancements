#!/usr/bin/env python3
"""Regenerate the PDF edition of the ngspice `optimize` user manual.

Usage:  python3 docs/internals/ngspice_internals/build_optimizer_pdf.py
Needs pandoc + xelatex (and the figures under ngspice_optimizer_figs/, which
make_optimizer_figs.py produces). Repo-relative *links* become GitHub URLs;
*image* paths are kept local so xelatex embeds the PNGs.
"""
import datetime
import os
import re
...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = subprocess.run([
            ...
            "-V", "title=ngspice commands — a worked reference",
            "-V", "subtitle=Every command in this ngspice-46 + OpenVAF-r build, with netlists and plots",
