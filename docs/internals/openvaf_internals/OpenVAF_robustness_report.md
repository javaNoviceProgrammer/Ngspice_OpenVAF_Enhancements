...
production-model corpus, ~50 adversarial hand-crafted inputs, and 4,000
...
real work, and the campaign surfaced four ways to make it crash or hang on
...
| | |
|---|---|
| Production models compiled | **117** — 0 crashes / hangs / panics |
| Adversarial hand-crafted inputs | **~50** — all rejected cleanly, 0 accepted-invalid |
| Mutation-fuzzing iterations | **4,000** — 0 panics, 0 segfaults |
| Crash / hang paths found | **4** — all fixed ([E-147](../../../enhancements_doc/Enhancement-147.md), [E-148](../../../enhancements_doc/Enhancement-148.md)) |
...
diode_cmc, …) was compiled — 117 files in all.

> **Result:** 117 / 117 compiled with **0 crashes / hangs / panics**. (A handful of
> include-fragment files — function libraries meant to be `` `include ``d, not
> compiled standalone — report a clean error, as expected.)
...
The three low-severity findings all require input a human or real model would never
write. The critical one is different: ~30 nested conditionals is easily produced by
macros in a real device model, and it *hangs* the compiler — the worst failure mode.
...
- **Corpus head-to-head.** Every one of the 117 production models compiles to the
  **identical verdict** before and after each fix — 0 flips — and BSIM4 (12.6 k
  lines) still compiles in ~2.3 s.
