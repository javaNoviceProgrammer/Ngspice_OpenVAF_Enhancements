...
**Where a parameter lives (Enhancement-546).** `collect_modules` also decides, per parameter, which side of the model/instance split it is on — and that split is an OpenVAF convention, not a language one: in Verilog‑A every parameter belongs to the instance, and a "model" parameter is one the compiler may resolve *once per card* because nothing in it varies between the card's instances. `(* type="instance" *)` is the author's word; `module_info::promote_instance_dependent` is the compiler's check on it. A parameter whose **default** reads an instance parameter — transitively, through user functions the default calls and function‑local parameters, with `$param_given(p)` counting as a read of `p` (`BodyRef::param_reads_and_calls` walks the inference result rather than `get_expr`, so it is total) — has a value per instance and is promoted (`is_instance = true`), which the whole back end then follows: instance storage, the per‑instance resolution in `setup_instance`, the OSDI table. The `instance_dependent_parameter` lint (L028, warn) names the promotion, except for an untyped `localparam`. A parameter whose **bounds** read an instance parameter keeps its level (BSIM6's `XGL from (-inf:L*LMLT+XL)` and HiSIM2's `LP from [0:L]` are model parameters and stay so) and is marked `instance_bounds`: `HirInterner::insert_param_init` takes the list twice — as `unchecked` in the model setup, whose range check would judge the card's value against card‑level neighbours, and as `check_only` in the instance setup, which range‑checks the model's value with the instance's values and stores nothing; `osdi::setup::setup_instance` routes that parameter's `ParamInfoKind::Invalid` callback to the model‑parameter error id (after every instance parameter, as `setup_model` numbers them) so the simulator names the instance and the value. Before Enhancement‑546 both dependences were resolved once per card, against the instance parameter's declared default (compiler hunt F2).

**Givenness and a default that can leave its range (Enhancement-555).** Two more facts `collect_modules` records per parameter. `given_tested`: the module's analog blocks, or a user function they call, test `$param_given(p)` (`BodyRef::param_given_tests` walks `resolved_calls` for the builtin; `module_given_tests` follows the calls). A statistical parameter with that flag is marked `OSDI_DIST_GATED` in the `OSDI_STAT_PARAM_INFOS` record, and the simulator draws it only when the deck gave it — because a draw is a write, the descriptor's `access()` marks every written parameter given, and a model that derives an ungiven parameter would switch branch instead of varying. And `dynamic_bounds`: the `from`/`exclude` bounds read some module-level parameter (the bounds half of the E-546 read analysis). `insert_param_init` takes those as `check_default` and judges the *default* against the bounds when the parameter is not given — the range can have moved since the default was declared (`l = 1.2 from [lmin:inf)` with `lmin` altered, swept or drawn) — while a constant default outside a constant range keeps Enhancement-56's exemption (the CMC "feature off" idiom, named by lint L027 at compile time). The `osdi` crate also exports, beside the descriptors, `OSDI_PARAM_GIVEN_FNS`: one `param_given_<sym>(inst, model, id, op)` per module (`osdi/src/given.rs`, built next to `access` from the same bitfield helpers, `bitfield::clear_bit` added), op 0 reading the flag, 1 setting, 2 clearing, `inst == NULL` meaning the card-level flag of an instance parameter. The simulator's sweep restores clear the flag of a parameter the deck never gave; the descriptor ABI is unchanged and an older object has no such symbol.

**The declared range, as text (Enhancement-558).** `Parameter::bounds_source` renders a parameter's constraints the way the source spells them — `from [lmin:inf) exclude 0` — from the body source map (`expr_map_back` gives each bound expression's syntax node); `ModuleInfo` keeps it as `range_text`, `sim_back` interns it (the codegen's `const_str_uninterned` looks a literal up in the table, it does not create one), and the `osdi` crate exports `OSDI_PARAM_RANGE_COUNTS` / `OSDI_PARAM_RANGES`, one C string per parameter in `param_opvar` order, `NULL` for a parameter without a range. The simulator's out-of-bounds message appends the text and the current value of every parameter the text names — `Parameter l of 'mm' is out of bounds (value 1.2; range from [lmin:inf), lmin = 1.5)` — so the parameter that moved the bound is finally in the message (hunt F9 of 2026-09-05).
...
- **Model data** — one per `.model` card. Holds the model‑level parameters (those *not* marked `type="instance"` — and, since Enhancement-546, not promoted: a parameter whose default reads an instance parameter is an instance parameter whatever its attribute says), the "given" flags (which parameters the netlist actually set), and cached parameter‑derived values.
