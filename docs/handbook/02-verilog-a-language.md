...
| Reserved parameter names | A module parameter named `m`, `temp`, `dtemp` or `dt` receives the netlist's `m=`/`temp=` and ngspice's own meaning is lost silently — the multiplier no longer applies, the instance temperature never reaches `$temperature`. Lint L029 `reserved_parameter_name` (warn, case-insensitive) says so; rename it, or for `m` apply the multiplier yourself. | [E-588](../../enhancements_doc/Enhancement-588.md) |
...
| Feature | Notes | Since |
|---|---|---|
| `@(initial_step)` / `@(final_step)` | Genuinely gated (once at the start; once at the end of a *successful* analysis, seeing the converged solution — the classic "report a tracked peak" pattern works). An op fires both. `examples/initial_step_examples/`, `examples/finalstep_examples/`. Since E-648 an event control under a **non-constant `if`/`case`** is refused (LRM 5.8: only a constant condition — a literal, a parameter, `analysis()`, `$temperature` — may guard one; `cross`/`above` keep 5.10.3.1's loop rule): put the event at the top level and test the condition inside its body. `examples/eventcond_examples/` | [E-7](../../enhancements_doc/Enhancement-7.md), [E-53](../../enhancements_doc/Enhancement-53.md), [E-648](../../enhancements_doc/Enhancement-648.md) |
...
| `$simparam("name"[, default])`, `$simparam$str("name"[, default])` | Numeric and string simulator parameters; ngspice exposes `analysis_name` and `simulator` among others. The string form takes the LRM's optional default since E-598, so `instance`/`module`/`path`, which ngspice does not serve, can be asked for without a `$fatal`. `examples/simparamstr_examples/`, `examples/simparamstrdef_examples/` | [E-25](../../enhancements_doc/Enhancement-25.md), [E-598](../../enhancements_doc/Enhancement-598.md) |
