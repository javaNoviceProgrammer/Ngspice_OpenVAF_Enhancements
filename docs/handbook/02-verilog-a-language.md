...
| `(* std=σ *)` / `(* std_rel=f *)` / `(* dist="gauss"\|"uniform" *)` on a **parameter** | Declares the parameter's statistics for `.option osdimc` automatic Monte-Carlo (see [§3.6](03-ngspice-workflows.md#36-monte-carlo)); scalar real, non-local parameters only, and combining `std` with `std_rel` is an error. The sigma is a numeric literal, or a quoted number (`std="25.0"`) — anything else in the quotes (`"25 ohm"`, `"inf"`) is a located error (`examples/osdimc_examples/`) |
