...
| 407 | feat: a `genvar` for-loop inside an `analog` block is unrolled at elaboration, which is how the LRM writes "contribute to every bit of a vectored net" (a bit-select must be a constant, so a run-time `integer` loop cannot express it at all); the LRM's own pages 91, 117 and 134 move from `limitations/` to `va/`, with page 117's rolled `dac` checked against its hand-unrolled `dac8` | [doc](../../enhancements_doc/Enhancement-407.md) | [genvarloop](../../examples/genvarloop_examples/) |
