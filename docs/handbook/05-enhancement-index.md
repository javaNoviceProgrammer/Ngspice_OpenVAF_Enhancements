...
| 116 | ngspice KLU **wrong-DC fix** for decoupled OSDI nodes — an OSDI internal node in no Jacobian entry (a `ground` reference whose `V(p,gnd)<+…` drops its column) was given an all-zero solver row, structurally singular under KLU (`groundcontrib` read `v(p)=0`, `hierbranch` branch currents `0`); now tied to ground at setup. Fixes 2 of the 3 numerical `KLU_XFAIL`s, leaving only the `opamp741` stiff transient | [doc](../../enhancements_doc/Enhancement-116.md) | [groundcontrib](../../examples/groundcontrib_examples/), [hierbranch](../../examples/hierbranch_examples/) |
| 117 | ngspice **periodic steady state (PSS) productionized** — `.pss` was experimental (`--enable-pss`, so unimplemented in shipped builds) and printed ~230 trace lines/run; now built by default, trace gated behind `set ngdebug` (232 → 31 lines), verified against the analytic AC response. Sparse-domain: a fail-fast guard directs `.pss` to `.option sparse` (it hangs under KLU). Foundation for the RF periodic small-signal suite | [doc](../../enhancements_doc/Enhancement-117.md) | [rfpss](../../examples/rfpss_examples/) |
| 118 | ngspice **PSS runs under KLU** — `.pss` hung under KLU: `klu_refactor`'s reused pivots inflated the truncation error into a ~20M-step timestep explosion. Forcing a full re-factor (`NISHOULDREORDER`) each PSS step makes KLU converge to the *same* result as Sparse; E-117's Sparse-only guard removed, so PSS now runs under both solvers | [doc](../../enhancements_doc/Enhancement-118.md) | [rfpss](../../examples/rfpss_examples/) |
| 119 | ngspice **retain the PSS periodic operating point** — PSS sampled node voltages per period then freed them and never captured device states; now voltages **and** states are retained per sample on the job (with frequency + dims) as the substrate the periodic small-signal suite (PAC/pnoise/PXF) linearizes around. First step toward PAC | [doc](../../enhancements_doc/Enhancement-119.md) | [rfpss](../../examples/rfpss_examples/) |
