...
*Fixed in [E-716](../../enhancements_doc/Enhancement-716.md).* The card-supplied zero takes the modulus' route, `OSDI(fatal) … /: the second operand (the divisor) is zero, so the integer quotient has no value`, and the analysis aborts; the genuinely run-time zero keeps E-518's defined quotient, the real division is untouched.
...
*Fixed in [E-717](../../enhancements_doc/Enhancement-717.md).* The quotient rule reuses the quotient and divides by `g` once, and the last division is emitted without the derivative fast-math flags — the first cut still read NaN because LLVM's combiner, licensed by `reassoc` and `arcp`, had re-formed `g·g`. Found on the way and left as a note there: the derivative code's `Partial` fast-math mode sets `nnan` where its comment says `contract`.
