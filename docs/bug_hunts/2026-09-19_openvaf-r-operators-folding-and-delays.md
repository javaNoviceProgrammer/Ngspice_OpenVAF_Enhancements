...
| [F4](#f4--simparamstr-of-an-unknown-name-hands-strobe-an-invalid-string) | *(fixed in [E-676](../../enhancements_doc/Enhancement-676.md): the stdlib returns the empty string after the fatal; the `�` was a deliberate U+FFFD sentinel, not a stray pointer)* `$simparam$str("instance")` (or any name the simulator does not provide, no default) hands `$strobe` an invalid string — `instance=�` on stdout — before the run aborts with the run-time `$fatal`; the compile-time L025 warning is right, the value that reaches the model is garbage | memory hygiene |
...
*Fixed in [E-676](../../enhancements_doc/Enhancement-676.md): `osdi/stdlib.c`'s `simparam_str` returned
the literal `"�"` (U+FFFD, a deliberate sentinel — not an invalid pointer or uninitialised bytes, as guessed
below) after raising the fatal; it returns the empty string, the twin of the numeric form's 0.0. The
evaluation still runs to its end after the fatal (OSDI code cannot unwind), with a well-defined value.
Pinned in `simparamstrdef_examples` (one check, which fails on the E-670 binaries). An unconditional
`$strobe` in that evaluation is dropped with the never-accepted iteration on both binaries; the
initial-step one the probe used prints at once.*
...
*Fixed in [E-677](../../enhancements_doc/Enhancement-677.md). Wider than the row: `OSDIfinalStep` evaluated
at `CKTrhsOld`, which after a frequency sweep holds the last frequency's small-signal solution, so the whole
final-step body ran on it — `V(a,b)` read 1 (the unit AC source's response) at a 0.2 V bias, and the
`last_crossing` synthetic node, with no small-signal stimulus, read 0. E-412 had protected the operating-point
variables by snapshot, not the evaluation. The OSDI load now keeps the converged operating point captured at
the small-signal initialisation, and the final step under `ac` and `noise` evaluates on it. Pinned in
`finalstep_examples` (`fsop`, three checks; the `ac` and `noise` ones fail on the E-670 binaries).*
...
*Fixed in [E-679](../../enhancements_doc/Enhancement-679.md). The node is held by the installed gmin in every
mode by design (E-595: no conductance or capacitance reaches it), and each delayed conductance makes its own
past its present current, so every delay multiplies the voltage by 1/(gmin·R). At each accepted transient point
the always-held list is checked, and a node past 1e15 V ends the run: `the solution is diverging -- node 'mid'
has reached -1e+18 V`, with the cause and the remedy. The hierarchical spelling was not silent after all — through
a control block ngspice prints the abort line and exits 0, its convention — but its cause was "unrecorded":
`NIiter` now records the singular row as the trouble node and the line names `n1#mid`. Pinned in
`dcpath_examples` (five checks; four fail on the E-670 binaries).*
