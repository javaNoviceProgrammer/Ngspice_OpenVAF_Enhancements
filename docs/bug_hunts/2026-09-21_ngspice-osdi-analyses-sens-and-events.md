...
| # | finding | kind |
|---|---|---|
| [F1](#f1--a-sens-writes-every-parameter-it-perturbed-back-as-given-the-temperature-dependence-the-temperature-and-the-resistors-ac-value-are-frozen-afterwards) | *(fixed in [E-685](../../enhancements_doc/Enhancement-685.md): the instance struct is snapshotted around each perturbation and put back byte for byte, as E-440 does for the model — the given flags, the OSDI instance data and its `$param_given` state return to what the netlist set)* after any `sens`, every instance parameter the sweep perturbed is written back **given**: a built-in resistor gets `tce=0` given and loses its `tc1`/`tc2` temperature dependence for the rest of the session (1.0 V at 60 °C where a fresh deck gives 1.33 V, even after `alter r2 temp=60`), an OSDI instance gets `temp` given and is pinned to the temperature of the moment (`set temp=80`, `.option temp`, `dc temp` no longer reach it; `alter n1 dtemp=10` is refused with "Instance temperature specified, dtemp ignored"); after an AC `sens` the resistor's `ac` alias is given and `alter r1 r=2k` followed by `ac` still reports the 1 kΩ response (0.4995 where 0.3327 is right, `op` is right); a second AC `sens` reports every resistor's sensitivity as −0; and an OSDI module's `$param_given(w)` turns true for a `w` the netlist never set, so a "derive from geometry if given" rule fires and a 1 kΩ device reads 2 kΩ; `reset` clears it all | wrong result, silent |
...
| [F3](#f3--analysisac-is-0-at-the-operating-point-of-sp-pz-disto-tf-and-an-ac-sens) | *(fixed in [E-684](../../enhancements_doc/Enhancement-684.md): the operating points of `sp`, `pz`, `disto` and an AC `sens` belong to "ac" — `analysis()`, `$simparam$str("analysis_name")` and the qualified step events alike; `tf` and a DC `sens` stay "dc")* at the operating point of an `sp`, `pz`, `disto`, `tf` or AC `sens` analysis the model reads `analysis("ac")` = 0 and `analysis("dc")` = 1, while the operating point of a plain `ac` reads `analysis("ac")` = 1 (E-53's owning-analysis rule); `@(initial_step("ac"))` does not fire under `sp` | conformance |
...
| [F9](#f9--probe-in3-on-a-four-terminal-osdi-device-saves-every-terminal-current-under-one-name) | *(fixed in [E-691](../../enhancements_doc/Enhancement-691.md): the explicit `i(<inst>)`, `i(<inst>,k)` and `i(<inst>,<terminal>)` probes on an OSDI instance go through the second pass E-628 made for `alli`, and carry the model's terminal names)* `.probe i(n3)` on a four-terminal OSDI device inserts four zero-volt sources (`vcurr_n3:nn:1_0` … `:4_0`) and saves their currents as four vectors that all carry the name `n3:nn#branch` (1e-3, −1e-3, 0, 0), so a script reads only the first; a two-terminal device gets one `n1#branch` and a built-in resistor `r1#branch` | wrong output naming |
...
*Fixed in [E-685](../../enhancements_doc/Enhancement-685.md).* Exactly that: the
perturbation loop snapshots the instance struct beside E-440's model snapshot and puts
it back after the write-back, at both restore sites. The resistor keeps its `tc1` (1.33 V
at 60 °C after a `sens`), the OSDI instance follows `set temp=80` and `alter dtemp`, the
`$param_given(w)` rule stays off, `alter r=` then `ac` reports the new resistance, a second
AC `sens` equals the first, and the noise after a `sens` equals the noise before it.
...
*Fixed in [E-684](../../enhancements_doc/Enhancement-684.md).* E-53's job consultation
(one helper since E-683, feeding the flags and the `analysis_name` string alike) names
`SP`, `PZ`, `DISTO` and an AC `SENS` "ac" at their operating point and their final step;
`@(initial_step("ac"))` fires under `sp`. `tf` and a DC `sens` are computed at zero
frequency and stay "dc"; the compiler accepts no other name, so nothing finer is
expressible.
...
*Fixed in [E-686](../../enhancements_doc/Enhancement-686.md).* Lint L037
`mfactor_double_scaling` (warn): a pass taints every variable assigned from a `$mfactor`
read and reports each flow contribution whose value carries one, at the read, with LRM
6.3.6 and `parares` as the help; `-E` makes it the error the LRM's example announces.
...
*Fixed in [E-687](../../enhancements_doc/Enhancement-687.md), with a correction.* The
`showmod` rows above were read from `showmod mt1 mt2 mt4` — three cards in one command —
and the listing printed the first card's defaults only; `showmod mt1` alone lists `temp 60`,
so the card path never hid them. E-687 prints one block per card of the group and refuses
`altermod <model> m=` in the card's words (E-426's decision, which stands).
...
*Fixed in [E-688](../../enhancements_doc/Enhancement-688.md).* `OSDIcollapsedNode` reads the
instance's node mapping; the deferred `.ic`/`.nodeset` is applied to the node `ai` collapsed
into, with a Note once, and the save warning names it ("save v(a) instead"). A name that is
no internal node keeps the old refusal; the empty-list "no data saved" stop is unchanged.
...
*Fixed in [E-691](../../enhancements_doc/Enhancement-691.md).* The explicit forms are kept
by their text during the deck read and spliced in `inp_probe_osdi`, where the model's
terminal names are known: `n3:p#branch`, `n3:n#branch`, `n3:cp#branch`, `n3:cn#branch`;
`i(n3,2)` and `i(n3,p)` one of them; a bus line in autobus shorthand written out first.
