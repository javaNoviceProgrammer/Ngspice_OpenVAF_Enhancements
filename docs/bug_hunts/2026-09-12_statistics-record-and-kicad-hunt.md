...
| F7 | **A `savemc` directory that does not exist is announced and then nothing is written** — "recording … to ./nodir/p8a.csv", no error, no file, every row and `writemc` value lost. | medium — silent loss |
