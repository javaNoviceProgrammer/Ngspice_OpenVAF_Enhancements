...
| F9 | *(the misattribution half fixed in [E-624](../../enhancements_doc/Enhancement-624.md): a `writemc` after the paused run is refused, naming its plot; the row is still missing)* **An analysis stopped at a breakpoint gets no row, and every `writemc` after it lands on the previous run's row**: `op`, `stop when time > 2u`, `tran`, three `writemc` → one row (the `op`'s) with the last value. The trial's draws are never recorded. | medium |
