...
| [F3](#f3--a-corner-named-tt-is-accepted-and-unreachable-and-the-loop-runs-the-nominal-twice) | *(fixed in [E-662](../../enhancements_doc/Enhancement-662.md))* a corner named `tt`, `nom` or `nominal` in a model is accepted by the compiler and unreachable in ngspice; the `corners` loop then lists the nominal twice; `-list` duplicates are not folded | silent no-op |
...
| [F5](#f5--l030-folds-an-integer-division-as-real) | *(fixed in [E-664](../../enhancements_doc/Enhancement-664.md))* L030 folds an integer division as real: `parameter integer a = 7/2` is warned as "the default 3.5", the value is 3 | wrong diagnostic |
...
*Fixed in [E-666](../../enhancements_doc/Enhancement-666.md): the raw file keeps
every corner's plot, named; `meas` and `writemc` read the combined plot (`writemc`
onto every corner's row); the copies are `v1_ss#branch` and `@rm_ss[rsh]`; the
devices follow the `corner` variable when the loop ends.*
...
*Fixed in [E-669](../../enhancements_doc/Enhancement-669.md): `.option corner=`
and `corners -list` look a name up directly; the loops take 255 beside `tt` and
say when a circuit declares more; the compiler refuses a name longer than 79
characters. The 80-character case was worse than described: `cp_getvar`'s
string read wrote one byte past every caller's buffer, and the name aborted
ngspice with a smashed stack; the reader fits its buffer now, and a name the
256-wide corner buffer cannot hold is refused as unselectable.*
