...
| [F7](#f7--compile-time-grows-quadratically-with-the-size-of-a-table-array-or-coefficient-list) | *(fixed in [E-713](../../enhancements_doc/Enhancement-713.md) for the array-literal, noise-table, parameter, variable, event and repeated-table rows -- five quadratic walks in the front end, the descriptor and LLVM's backend on one giant block; a 20 000-element localparam array 1.0 s, 5 000 noise pairs 0.5 s, 500 cross events 0.5 s, 50 000 variables 3.1 s; the loop rows are 41 s and 7 s at 20 000 and 10 000 elements and still superlinear -- with the laplace_nd row fixed in [E-712](../../enhancements_doc/Enhancement-712.md) and the inline table rows in [E-705](../../enhancements_doc/Enhancement-705.md))* compile time is quadratic or worse in the size of an array or list: a `localparam` array of 20 000 values takes 15.5 s and 100 000 does not finish in 300 s; a loop that fills a 20 000-element array takes 107 s (40 000 does not finish in 400 s) and one that only reads a 10 000-element array 27 s; an inline `noise_table` of 5 000 pairs takes 58 s (100 000 pairs, 300 s, unfinished); a `laplace_nd` denominator of 1 000 coefficients 17 s (100 000, unfinished) | pathological compile time at realistic sizes |
...
*Fixed in [E-713](../../enhancements_doc/Enhancement-713.md)* (with [E-705](../../enhancements_doc/Enhancement-705.md)
for the table rows and [E-712](../../enhancements_doc/Enhancement-712.md) for `laplace_nd`). The "where"
above was a guess, and wrong: none of these rows was the cost of every value being a
register. Re-measured one by one they were five separate quadratic walks — every element of
an array literal re-flattening the whole literal in the front end; a linear scan of the
item map per item lookup and a re-walk of a declaration's attributes per variable it
declares (the 50 000-variable row); the descriptor's given-flag functions as a `switch`
with a case per parameter; a noise table as a chain of selects that LLVM's instruction
combiner is quadratic in; and an eval function whose straight-line code is one giant basic
block, which three LLVM backend passes are quadratic in (the SelectionDAG list scheduler
for the `cross` events, the machine scheduler for the array loops, the greedy register
allocator for the repeated table calls). Each has its own fix; the giant-block eval is
emitted through the fast instruction selector above 12 288 SelectionDAG nodes, more than
twice the largest compact model's block. The module chain is not a compiler defect: each
level holds the whole chain below it, so a 1 000-deep chain is 500 000 resistors; the
200-deep one compiles in 6.7 s. The loop rows remain superlinear (a 40 000-element fill
163 s, a read 59 s, both unfinished before): what is left is LLVM's instruction combiner and
the MIR passes on a 40 000-phi loop, and the real fix is a memory-backed array for a
run-time index.
