...
- `laplace_nd/np/zd/zp` via exact state-space realization and
  ...
  parameter-dependent coefficients: ✅. A z-filter root **at the origin** is
  the factor `z` of 4.5.12 ("implemented as z, rather than (1 − z⁻¹ r)"):
  a `zi_zp`/`zi_zd`/`zi_np` pole at zero is the one-period delay and a zero
  the one-period advance, as the `zi_nd` spelling of the same filter already
  was ([E-699](../../enhancements_doc/Enhancement-699.md); the root expansion built the term 1 — a wire — until then). The
...
- `$table_model` (LRM 9.21; an earlier revision of this document cited
  ...
  **N+M-column isoline format** — **ragged isolines included** (the
  ...
  `I` on the runtime 1-D array form and on inline `'{...}` data, and `D`
...
- since [E-651](../../enhancements_doc/Enhancement-651.md) every one of these
  projections -- a noise power, a flicker exponent, a standard deviation, a
  uniform range -- is **named** when the deck fixed the value: a deferred
  `$warning`-style line gives the builtin, the domain, the number and the
  substitute, once per accepted point. The projection itself is unchanged, the
  exponential family keeps its mandated fatal, and a run-time quantity is still
...
| 4.5 Analog operators (all), **clause-by-clause audit 4.5.1–4.5.15** ([E-514](../../enhancements_doc/Enhancement-514.md)); a z-filter root at the origin is 4.5.12's factor `z` ([E-699](../../enhancements_doc/Enhancement-699.md)); frozen-`td` absdelay implemented, ddx over unnamed-branch flows, no-ic `idtmod` pins DC at 0, `default_transition` honored for explicit zeros | ✅ (`limexp` stateless ⚠️, idt-reset τ=10µs ⚠️, ddt/idt tolerances not plumbed ⚠️, zi continuous-bilinear ⚠️, dynamic laplace coeffs track+warn ⚠️ — all documented) | `absdelay`, `laplace`, `zi`, `slew`, `transition`, `idt*`, `ddx`, `opargs`, `discontinuity`, `last_crossing`, `defaulttransition`, `transedge`, `rtdomain`, `deckdomain` |
...
| 5.10 Events (steps with Table 5-1 exact per analysis, cross/above/timer, cross DC/t=0 gating, above init event, OR lists incl. comma, phase lists, placement rules enforced, invalid events rejected) | ✅ (cross/above tolerances ⚠️ warned not honored; strict out-of-range dir ⚠️ documented); any event under a non-constant if/case refused per 5.8 (E-648) | `initial_step`, `finalstep`, `cross`, `timer`, `lrmcorner` |
| 4.7 Analog functions (arrays in/out/return incl. the LRM's Example 3 spelling and a parameter array as the argument (E-645; a size mismatch and an output formal fed a parameter are refused by name); VAMS-2023 string return & args; function-local `parameter`s with shadowing; output-array zero-init; locals fresh per call (E-646); a real actual into an integer input rounds (E-647); `return` statement) | ✅ (named blocks in bodies ⚠️ and const-context calls ⚠️ documented relaxations) | `funcarray`, `arrayout`, `arrayret` |
...
| 9.13 Random/distributions (9.13.2 domain rules errors on the deck route too; a deck-fixed negative standard deviation or reversed uniform range is projected **and named**, [E-651](../../enhancements_doc/Enhancement-651.md); `type_string` scoped to paramsets) | ✅ (deterministic seed ⚠️ documented) | `rng`, `montecarlo` |
