...
| 4.7 Analog functions (arrays in/out/return incl. the LRM's Example 3 spelling and a parameter array as the argument (E-645; a size mismatch and an output formal fed a parameter are refused by name); VAMS-2023 string return & args; function-local `parameter`s with shadowing; output-array zero-init; `return` statement) | ✅ (named blocks in bodies ⚠️ and const-context calls ⚠️ documented relaxations) | `funcarray`, `arrayout`, `arrayret` |
