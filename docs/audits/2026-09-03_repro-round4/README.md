...
| deck | finding |
|---|---|
| `discovr.va` / `discovr.cir` | **§3** — a discipline's `flow.abstol = 10u` override is dropped: `a.flow.abstol` and the nature derived from `ttl.flow` both read `1e-6`. **Fixed** (the override's value is evaluated at all now, a discipline attribute is a resolvable item that attribute access consults before the nature, the 3.6.2.6 parent link reads the discipline at the hop, and `osdi_node_abstol` walks a discipline's own attribute range) — the deck prints `1e-05` on both lines on the fixed pair and reproduces only against the audited binaries. Pinned by nine new checks in `examples/lrmdisc_examples/` |
