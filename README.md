...
| 425 | fix: a corrupt row in a `$table_model`/`noise_table` data file was silently dropped — a tenfold wrong answer, and in the N-dimensional form the whole table contributed exactly zero; plus a real literal that overflows to infinity and a zero-size based literal | openvaf-r | [doc](enhancements_doc/Enhancement-425.md) | [tabledata](examples/tabledata_examples/) |
| 426 | fix: inputs that were never checked — a mistyped analysis output node was invented (and read off the end of the RHS array), `.ac`/`noise`/`sp`/`.dc` ran a different sweep than the one asked for, `meas` on an unsaved `@dev[param]` answered 0.0, a sub-absolute-zero temperature gave a negative thermal voltage, `itl2=0` cost 13,400x the iterations, `m=-1` inverted a device, `1e400` became inf, and a failing OSDI `setup_model` masked by a later success was a SIGSEGV | ngspice | [doc](enhancements_doc/Enhancement-426.md) | [inputguard](examples/inputguard_examples/) |
| 427 | fix: a `.dc @inst[param]` sweep applied values the model's Verilog-A `from` range forbids and published the results (rc=0) while every other path refused them; the sweep also handed the device one value past `stop`, so a sweep ending at a range edge complained; a `@(timer)` event landing exactly on `tstop` never fired because the schedule accumulates by repeated addition; and an integer instance parameter could not be swept at all | both | [doc](enhancements_doc/Enhancement-427.md) | [sweepparam](examples/sweepparam_examples/) |
| 428 | fix: a Verilog-A device's INTERNAL node inside a subcircuit can be read the obvious way — `v(x1.n1#mid)` as well as `v(n.x1.n1#mid)` — in every consumer; the device-type letter was a subcircuit-flattening artefact leaking into a node name, and `.save` failed destructively, discarding the whole analysis output rather than one vector | ngspice | [doc](enhancements_doc/Enhancement-428.md) | [hiernode](examples/hiernode_examples/) |
| 429 | fix: a `.tf` or `.noise` CARD naming a node that does not exist answered `transfer_function = 0.000000e+00` with no diagnostic — E-426's bounds guard only fires for the `.control` path, because a card's invented node is created before the matrix is sized and sits inside it; a node an analysis card invented and nothing else ever referenced is now refused, while a card legitimately placed before its devices still resolves | ngspice | [doc](enhancements_doc/Enhancement-429.md) | [inputguard](examples/inputguard_examples/) |
| 430 | fix: `.probe`'s refusal of a token now names what it accepts and points an `@device[param]` at `.save`/`.options savecurrents` — the refusal itself is correct, since `.probe` measures by inserting series voltage sources while `@device[param]` is read from the device; the warning also printed twice per token, echoed the internal `*probe` form, and misspelled "ignored" (5 further occurrences fixed elsewhere) | ngspice | [doc](enhancements_doc/Enhancement-430.md) | [probeshort](examples/probeshort_examples/) |
| 431 | fix: a `sweep -output` naming a vector that never resolves was recorded as a full column of zeros — a plottable, entirely fictional flat line — because `sw_eval_expr` returns 0.0 on failure, indistinguishable from an expression that is legitimately zero; it is now named in an error and its curve is not emitted, while a genuine zero and any surviving sibling outputs are kept | ngspice | [doc](enhancements_doc/Enhancement-431.md) | [sweepguard](examples/sweepguard_examples/) |
| 432 | fix: `sweep -output` now takes every expression up to the next flag, as its usage line has always advertised — only the first token was read and the rest were reported as unrecognized tokens, leaving the sweep running with a shorter output list; the list ends at the flags `sweep` knows rather than at any leading `-`, so a negated expression stays an expression. Also fixes an Enhancement-431 regression: the per-output failure tally was zeroed over the outputs known at parse time, so auto-collected outputs (the no-`-output` default) inherited stack garbage and real curves were dropped | ngspice | [doc](enhancements_doc/Enhancement-432.md) | [sweepguard](examples/sweepguard_examples/) |
| 433 | fix: consistency in two places. `sweep`/`optimize`/`montecarlo`/`highsigma`/`wcd` never called `cp_unquote`, so a double-quoted `-analysis` or `-output` argument arrived with its quotes attached (ngspice's lexer strips `'...'` itself but keeps `"..."` in the word, leaving each command to remove them); the same flag was also collected three different ways, so `wcd` alone required quoting and a negative analysis argument broke `montecarlo`/`highsigma`. Separately, a `.model` inside a subcircuit is renamed `<path>:<model>`, so the natural `@x1.rmod[res]` was refused where devices and nodes both take a dot -- now mapped onto the colon form for `altermod`, `optimize -mparam` and readback alike | ngspice | [doc](enhancements_doc/Enhancement-433.md) | [sweepguard](examples/sweepguard_examples/), [hierdev](examples/hierdev_examples/) |
| 434 | fix: three silent wrong answers plus a silent truncation, from the round-35 hunt. `$simparam("temp")` was not supplied though `tnom` was, so it returned the caller's default or killed the run with OSDI(fatal); `$simparam("abstime")` returned the DC sweep VOLTAGE during `.dc`; montecarlo/highsigma/wcd silently truncated `-analysis` at a 512-byte buffer and ran a different command; and `.tf` was the one analysis reporting a result while swallowing a model's `$finish`. A fourth finding -- a `save` list resolving to nothing aborts the analysis -- is documented but NOT fixed: the abort turns out to guard a latent long-name overflow | ngspice | [doc](enhancements_doc/Enhancement-434.md) | [silentloss](examples/silentloss_examples/) |
| 435 | fix: `sweep` now accepts a subcircuit-local model parameter written the way the rest of the hierarchy is written, `@x1.rmod[res]`. Enhancement-433 taught the name funnels that spelling and reached `altermod`, `optimize -mparam` and readback, but `sweep` classifies knobs on its own path and missed it -- and the failure was not a refusal: the sweep ran on with a knob that never moved, producing a full set of points and a plottable FLAT curve behind a diagnostic that scrolled past | ngspice | [doc](enhancements_doc/Enhancement-435.md) | [hierdev](examples/hierdev_examples/) |
| 438 | fix: a failed simulation no longer becomes data. `montecarlo` counted samples whose analysis never solved as PASSING -- 14 of 20 failures still reported 100% yield -- and they are now excluded and reported; `sweep` names the points that did not converge (a failed operating point reads back as 0, indistinguishable from a real zero); `optimize` scores a failed evaluation as worst-case and reports how many. Plus: a bogus co-knob no longer cancels the whole sweep restore, `.meas ... WHEN` no longer prints a fabricated 0 when it cannot resolve, an unknown `.options` name is reported, an unmatched `.if` sets the exit status, and a successful `hb` exits 0. New opt-in `.option warn_physics` flags non-physical device values (`abs(k) > 1`, negative `ron`/`l`/`w`/`bf`) that stay legal by default | ngspice | [doc](enhancements_doc/Enhancement-438.md) | [failacct](examples/failacct_examples/), [warnphysics](examples/warnphysics_examples/) |
| 437 | fix: a swept `@*:model[param]` is put back when the sweep ends -- E-436 gave the form a set path but not E-409's capture-and-replay, so every matched model was left at the last swept value and the name also reached the scalar reader, printing a spurious parse error. Also: a `.temp` card with no value ran the whole circuit at 0 C silently (15.6% wrong on a tempco divider); it is now refused, keeping 27 C, as its sibling `.options temp=` already did | ngspice | [doc](enhancements_doc/Enhancement-437.md) | [modelwild](examples/modelwild_examples/), [sweepguard](examples/sweepguard_examples/) |
| 436 | feat: `@*:model[param]` -- one model name, every instance path. Subcircuit expansion renames a `.model rmod` inside x1 to `x1:rmod`, so `@rmod[param]` reached only the top-level card while every subcircuit copy silently kept its old value, and `@*[param]` swept up unrelated models; the new form matches the leaf name at any depth INCLUDING none, so the top-level card and every flattened copy move together. `@*.model[param]` accepted too. A bare `@rmod[param]` still means the top-level card alone -- broadening it would silently change existing decks -- but now reports how many copies it left untouched | ngspice | [doc](enhancements_doc/Enhancement-436.md) | [modelwild](examples/modelwild_examples/) |
| 424 | fix: a noise or `ac_stim` source inside a run-time loop registered no source at all and contributed nothing, silently — while every other analog operator in that position was already an error; plus `$finish`/`$stop` diagnostic levels outside 0/1/2 | openvaf-r | [doc](enhancements_doc/Enhancement-424.md) | [noiseloop](examples/noiseloop_examples/) |
| 423 | fix: a parenthesised comma list `(a, b)` was an expression that kept only its first element and never checked the rest, so a `,` written for a `+` silently dropped a whole term — and undeclared names hid in the discards; plus a trailing comma in a call | openvaf-r | [doc](enhancements_doc/Enhancement-423.md) | [commaexpr](examples/commaexpr_examples/) |
| 422 | fix: one reference, three outcomes — an unresolvable parent nature CRASHED the compiler (even unused), `ddt_nature`/`idt_nature` went silent and a discipline's `potential`/`flow` blamed the model body; plus nature inheritance cycles and an unvalidated `abstol` | openvaf-r | [doc](enhancements_doc/Enhancement-422.md) | [natureref](examples/natureref_examples/) |
| 421 | fix: a check that existed for one spelling and not its sibling — an `exclude` that swallows its `from` range or is written backwards, and `$simparam`/`$simparam$str` names, the only such name whose typo is FATAL at run time; plus a second `default` arm per IEEE 1364 | openvaf-r | [doc](enhancements_doc/Enhancement-421.md) | [rangeguard](examples/rangeguard_examples/) |
| 420 | fix: six things accepted then silently degenerate — integer `**` with a negative exponent per IEEE 1364 Table 5-6, a zero laplace denominator, a zero `zi_*` sampling period, an illegal `last_crossing` direction, a `$discontinuity` degree below −1, an `ac_stim` analysis name that can never match | openvaf-r | [doc](enhancements_doc/Enhancement-420.md) | [vafdegen](examples/vafdegen_examples/) |
