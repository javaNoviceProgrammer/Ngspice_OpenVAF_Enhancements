...
| 199 | **N-port Touchstone device** (`snp2va.py`) — drop a measured/simulated S-parameter block (filter, cable, package, amplifier) from a Touchstone `.sNp` file into a sim. Converts `.sNp` → a Verilog-A n-port realized with **`laplace_nd`**, so via the OSDI laplace machinery it works in **AC *and* transient** (no convolution engine). Pipeline: parse → S(f) → Y(f) → **common-pole vector fit** (Gustavsen) → `I(p_i) <+ Σ_j laplace_nd(V(p_j),num,den) + e_ij·ddt(V(p_j))` (the improper `e·s`/shunt-C term split out as `ddt`). **Automatic order selection** (stops at the knee, so noisy data is fit at its noise floor not over-fitted); stability enforced (RHP poles reflected → BIBO-stable); passivity checked+reported. Pure Python **stdlib** (no numpy — Householder-QR lstsq, complex matrix inverse, Durand-Kerner roots included). Any port count; S/Y/Z; MA/DB/RI. Verified: device matches the original R-L-C resonator in AC (5e−6) and transient (5e−3); 3-port matches both coupled outputs (4e−9); noisy fit stays bounded. 5 checks | openvaf‑r + ngspice | [doc](enhancements_doc/Enhancement-199.md) | [nport](examples/nport_examples/) |
