...
| 396 | fix: ten openvaf-r defects -- `$limit` with any unresolvable name or arity (including `fetlim` spelled as the LRM documents it) left a NULL function pointer that the model then CALLED, **segfaulting ngspice with zero output**; a model parameter named `m` silently defeated [E-394](enhancements_doc/Enhancement-394.md)'s subcircuit multiplier, and one named `temp` shadowed the instance temperature; the built-in collision warning blamed letter case for a same-case clash; `$table_model` data files accepted `nan`/`inf`/`1e400` and poisoned the whole table; `@(timer)` with a zero or negative period fired on every evaluation; `$bound_step`, `noise_table` shape, `@(cross)` direction, `transition`/`slew`/`idtmod`/`absdelay` arguments and a bus port's two ranges were all unvalidated | both | [doc](enhancements_doc/Enhancement-396.md) | [limguard](examples/limguard_examples/) |
