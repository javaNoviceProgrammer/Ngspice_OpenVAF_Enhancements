...
<details>
<summary><b>📖 Show the full enhancement table</b> — 322 rows, click to expand</summary>
...
| 321 | PERF: extends the E-320 .param fast-sweep to SUBCIRCUIT-INTERNAL device values (previously always reset). A flattened instance card (r.x1.r1) carries card->linenum back to its subckt-definition body line, whose original {expr} numparam keeps in dynrefptr; so a walk of the flattened deck (ci_deck) + new nupa_get_dynref recovers every instance's value expression paired with its full hierarchical name -- top-level and subckt captured uniformly, no name reconstruction. Safe across subckt scope: disarms on local shadow (.param swept-LHS in a subckt), formal-param default or pass-through (X call passing the param), derived and structural uses; plus an arm-time self-check that the global re-evaluation reproduces the value numparam baked into the flattened card, so any missed shadow disarms. ~12x on a large circuit where the param feeds few subckt-internal devices; a 10-deck arm/disarm battery is bit-identical to the reset path in every case | ngspice | [doc](enhancements_doc/Enhancement-321.md) | [paramfastsweep](examples/paramfastsweep_examples/) |
| 322 | PERF: the `optimize` command shares the E-320/321 .param fast-path. Every -dparam (symbolic .param) objective evaluation previously did alterparam+reset (re-parse+rebuild the whole circuit) -- and an optimization runs tens to hundreds of evals. Now, when the deck-param knobs feed only in-place-able device/model values, each eval pushes the candidate's values in place via the shared sw_fp_build/sw_fp_apply (exported through com_sweep.h) -- no per-eval reset -- across Nelder-Mead, Levenberg-Marquardt, PSO, DE, SA and NSGA-II (opt_eval + opt_eval_objs). Same conservative classifier + arm-time self-check, so it disarms to the exact reset path on any unsafe use. Two opt-outs: -center design-centering keeps reset (its inner Monte-Carlo re-samples process variation, a re-draw not a value push); and a device-count guard (>=80 instances) keeps the already-cheap reset on small circuits, where the fixed per-eval numparam overhead would lose and an extremely tight -tol could otherwise diverge the iteration count. 5.6x on a 4000-device single-knob fit; NM/PSO/DE optima and the NSGA-II Pareto front bit-identical to reset; all 41 optimize verify checks still pass | ngspice | [doc](enhancements_doc/Enhancement-322.md) | [optimize](examples/optimize_examples/) |
