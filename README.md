...
| 140 | ngspice **oscillator phase noise** (`hbosc <oscnode> <K>` + `phasenoise <fstart> <fstop>`) — closes the Pnoise gap with the phase-noise piece. `hbosc` is an **autonomous HB**: an oscillator has no source, so `F(V)=I_R+[dq/dt]=0` is solved for the harmonics **and** the unknown frequency `ω0` — the conversion matrix `H` is singular (phase mode `u_k=jk V_k`), so Newton runs on the **bordered** system `[H, ∂F/∂ω0; u*ᵀ, 0]`, transient-seeded, converging quadratically (4 iters, `\|F\|=3e-12`; inductors fit via branch-current MNA). `phasenoise` builds the adjoint at OFFSET `df` with the unit at the **carrier sideband** and folds device noise; as `df→0` the limit-cycle matrix goes singular through the phase mode → noise diverges as `1/df²`, normalized to carrier power `2\|V1\|²` → `L(df)` in dBc/Hz. Verified 8/8 on an LC osc: converges to `f0`+describing-fn amplitude, **−20 dB/dec** skirt flattening into the floor, physical level (≈−147 dBc/Hz @1kHz), **thermal scaling L∝T** (2×T → +3 dB exactly), clean error, KLU==Sparse; pnoise stays 9/9 | ngspice | [doc](enhancements_doc/Enhancement-140.md) | [phasenoise](examples/phasenoise_examples/) |
| 141 | ngspice **two-tone QPXF** (`qpxf <output_node> <f_in>`) — the quasi-periodic transfer function, the **adjoint of QPAC**, completing the two-tone small-signal suite (QPSS→QPAC→QPnoise→**QPXF**, mirroring PSS→PAC→pnoise→PXF). One adjoint solve `Hᵀ Ψ = e_{out,(0,0)}` (`qp_solve_adjoint`, reused from QPnoise), each sideband block of `Ψ` dotted with the AC-source pattern `B0` → the transfer from an input at every sideband `f_in+k1·f1+k2·f2` to the output. By reciprocity `(H⁻¹B)_out=(H⁻ᵀe_out)ᵀB` the `(0,0)` transfer is **bit-identical** to the QPAC response — the PXF↔PAC cross-check. Reads only retained data → **solver-independent (KLU + Sparse)**. Verified 6/6: reciprocity (QPXF(0,0)==QPAC(0,0) bit-identical), sideband reciprocity, reduce-to-XF (pump→0 ⇒ `(0,0)`=plain transfer=R, sidebands vanish), clean no-op-point error, KLU==Sparse; QPSS/QPAC/QPnoise stay 11/11+7/7+7/7+10/10 | ngspice | [doc](enhancements_doc/Enhancement-141.md) | [qpxf](examples/qpss_examples/) |
| 142 | ngspice **QP small-signal frequency sweep** — `qpac`/`qpnoise`/`qpxf` gain a `<dec\|oct\|lin> N fstart fstop` sweep of `f_in`, emitting a plottable **ngspice plot** (conversion gain / noise figure / image-rejection curves), matching how `.ac`/`.pnoise`/`.pxf` sweep. Each point reuses the same single-frequency solve: `qpac` records per-node `\|(0,0) response\|` (like a pumped `.ac`); `qpnoise` records `onoise_spectrum`/`inoise_spectrum`; `qpxf` records `xf` (in-band) + `xf_conv` (total conversion). The analysis fills arrays; the front-end builds the plot via the nutmeg vector API (`plot_alloc`/`dvec_alloc`/`vec_new`) — the correct layer for a front-end command (the analysis-job output framework needs a live job a front-end command lacks). Verified 5/5: swept value at 0.3G == single-frequency `qpac`/`qpxf`/`qpnoise` (to machine precision), reactive roll-off vs `f_in`, correct point count; single-frequency suites stay 7/7+10/10+6/6 | ngspice | [doc](enhancements_doc/Enhancement-142.md) | [sweep](examples/qpss_examples/) |
| 143 | ngspice **least-squares curve fitting for `optimize`** — the built-in optimizer (E-130, Nelder-Mead) gains a gradient-based **least-squares** mode: one or more weighted `-target <expr> <value>` measurements, optionally spread over several `-analysis` stages (so one fit can combine a DC operating point **and** an AC response), are fitted with **Levenberg-Marquardt** (finite-difference Jacobian). Exploiting the sum-of-squares structure, it reaches the optimum in far fewer analysis runs than the simplex (**27 vs 67** evals on a two-target RC fit). Scalar `-minimize`/Nelder-Mead path unchanged; `-method nm\|lm` overrides the auto choice. All in `com_optimize.c` (no ABI change). Verified 23/23 (E-130 11 + E-143 12): 3-target/3-stage RC fit, 2-param DC+AC multi-analysis fit, LM-beats-NM eval count, **OSDI diode extraction** (recover both `is` and `n` from two I-V points), input validation | ngspice | [doc](enhancements_doc/Enhancement-143.md) | [fit](examples/optimize_examples/) |
| 144 | ngspice **`optimize` tunes symbolic `.param` values** — a second knob kind `-dparam` lets the optimizer vary netlist `.param` symbols (e.g. `.param w=1u`, `R1 in out {500*k}`), not just `alter`-reachable device/instance params (`-param`). Since a `.param` is expanded at parse time, `-dparam` changes it with `alterparam` + a quiet `reset` that re-sources the deck (re-evaluating the `.param` expressions, re-stamping device values). Deck params are re-sourced **first** and the in-place `alter` params re-applied after, so both kinds mix in one run; the re-source banners are gated by `ft_optimizing` (inner iterations silent). Circuits with no `-dparam` keep the E-130/-143 fast path. Closes the last E-143 follow-up. In `com_optimize.c` + `inp.c` (banner gate); no ABI change. Verified 31/31 (adds: scalar `.param` fit, **mixed** `.param`+`alter` fit, `.param`-in-expression, least-squares `.param`, quiet re-source; also AC + OSDI-value `.param` manually) | ngspice | [doc](enhancements_doc/Enhancement-144.md) | [fit](examples/optimize_examples/) |
