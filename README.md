...
Two hundred and twelve enhancements so far — language features, correctness fixes, systematic audits, and simulator-side workflow tooling, each verified end-to-end by a committed example suite and released with a detailed write-up.
...
| 190 | ngspice **nested multi-knob sweep** (`sweep … -vs …`) — the `sweep` command (E-146) stepped only one knob. `-vs <knob> <spec>` adds outer knobs whose cartesian product forms a curve *family*: the inner knob stays the x-axis and each outer combination gives one curve per output (`<out>_<knob>_<val>`), the parametric family of `.dc … SWEEP …`. Correct set-ordering (`.param` → `alterparam`+`reset` once per outer step, in-place `alter` re-applied after each reset). Single knob reduces exactly to E-146; `-overlay` composes; RC families match `1−exp(−T/RC)` to <5e-6; 5 checks | ngspice | [doc](enhancements_doc/Enhancement-190.md) | [nestedsweep](examples/nestedsweep_examples/) |
| 191 | ngspice **`.ac lin 2` / `.sp lin 2` off-by-one fix** — found while auditing the output path (a ragged `wrdata` test wrote a too-short AC vector). The AC (`acan.c`) and S-parameter (`span.c`) linear-sweep step was guarded by `numberSteps − 1 > 1` (true only for N≥3), so exactly `lin 2` fell into the single-point patch (`freqDelta=0`) and produced **one** frequency point instead of two — the `fstop` point was silently dropped. Fixed to `> 1` (matching the already-correct noise analysis), preserving the genuine `lin 1` single-point case. Both `lin 2` endpoints match `1/(1+jwRC)` to <1e-6; both solvers; 5 checks. (Audit found the writers themselves — Touchstone RI/MA/DB·S/Y/Z round-trip, `wrdata`, `save`, rawfile `write`/`load` — all clean.) | ngspice | [doc](enhancements_doc/Enhancement-191.md) | [aclin2](examples/aclin2_examples/) |
| 192 | ngspice **auto-checkpoint on interrupt** (`set autosave=<file>`) — E-131 `savestate` had to be run by hand, so interrupting a long transient lost its progress. `set autosave=<file>` makes a **Ctrl-C** (or any pause) auto-write an E-131 checkpoint. Hooked at the safe `E_PAUSE → err==1` point in `dosim` (main thread, at an accepted timepoint — **not** in the signal handler, so I/O is safe), reusing the `savestate` writer (factored into `ckt_write_checkpoint`). Opt-in, transient-only (`MODETRAN` guard); the autosave file is **byte-identical** to a manual `savestate` and `loadstate` resumes it. Verified incl. a **real SIGINT** through a PTY; 5 checks | ngspice | [doc](enhancements_doc/Enhancement-192.md) | [autosave](examples/autosave_examples/) |
| 193 | ngspice **`.pnoise` honors `sqrnoise`** (noise-density units fix) — found auditing the RF/PSS suite. The manual defines `onoise_spectrum`/`inoise_spectrum` as V/√Hz by default, switched to squared V²/Hz by `set sqrnoise`; `.noise` obeys this but **`.pnoise` always emitted V²/Hz and ignored `sqrnoise`**, so the same-named vectors differed by a *square* across the two analyses. Fixed `pnoise_sweep` (`dcpss.c`) to read `sqrnoise` and default to V/√Hz — now `.pnoise` == `.noise` by default (to 0.00). Both folding paths covered; pnoise-family examples (rc_pnoise/cyclo/cyclofold/pnoisefold) updated. (Audit also verified PAC ±1 conversion sidebands + HB vs transient DFT; documented the `.pac` trailing `maxsideband` arg.) 4 checks | ngspice | [doc](enhancements_doc/Enhancement-193.md) | [pnoiseunits](examples/pnoiseunits_examples/) |
| 194 | ngspice **particle swarm optimization** (`optimize -method pso`) — the built-in optimizer (E-130/143) had only *local* methods (Nelder-Mead simplex, Levenberg-Marquardt), which descend into whichever basin the start point sits in. `-method pso` adds a **global**, population-based, derivative-free search: a swarm flies through the box with the Clerc-Kennedy constriction, finding the **global** optimum of multimodal objectives that trap a local method. Works for scalar `-minimize` and `-target` least-squares; `-swarmsize`/`-seed` (self-contained reproducible PRNG). On `sin(p)+sin(10p/3)` from a trapping corner, PSO finds the global (−1.900) while NM sticks at a local (−1.20). 6 checks | ngspice | [doc](enhancements_doc/Enhancement-194.md) | [psoopt](examples/psoopt_examples/) |
| 195 | ngspice **differential evolution** (`optimize -method de`) — a second global optimizer method after PSO (E-194). DE builds each trial from a scaled *difference* of random population members (`v = a + F·(b−c)`) crossed with the target (DE/rand/1/bin, F=0.8 CR=0.9); the difference self-scales to the population spread, so it's robust on rugged/discontinuous landscapes. Shares PSO's `-swarmsize`/`-seed` (reproducible PRNG) and works for scalar + least-squares. On the same multimodal testbed DE and PSO both reach the global (−1.900) where NM sticks at a local (−1.20); DE hits p* to 5 digits. `optimize` now has a global toolbox (pso, de) beside local nm + gradient lm. 7 checks | ngspice | [doc](enhancements_doc/Enhancement-195.md) | [deopt](examples/deopt_examples/) |
| 196 | ngspice **simulated annealing** (`optimize -method sa`) — the classic single-walker global optimizer, completing the method set. From the current point it accepts an uphill move with probability `exp(−Δcost/T)` (Metropolis), climbing out of local minima while temperature T is high, settling as T cools geometrically to zero. One eval per step (no population) → lightest global method, ideal when each analysis is costly. Auto-scaled T0 (from random-probe cost spread) and step (shrinks as T cools); nothing to tune. Scalar + least-squares; `-seed` reproducible. On the multimodal testbed all three global methods (sa/pso/de) reach the global (−1.900) where local nm sticks (−1.20). `optimize` now spans local (nm, lm) + 3 global strategies (pso, de, sa). 7 checks | ngspice | [doc](enhancements_doc/Enhancement-196.md) | [saopt](examples/saopt_examples/) |
| 197 | ngspice **100-parameter optimization** — raised the `optimize` limits from 16 params / 64 targets to **128 each** (auto-population cap 60→256), so a large problem fits in one call. Testbed: 100 current-source→resistor nodes fitted to a 100-point voltage ramp. **Levenberg-Marquardt** solves the well-posed 100-D least-squares to machine precision (rms 5e−16, 619 evals, ~2 s). The global methods now function at scale too: DE's classic `CR=0.9` mutates ~every coordinate, so in high-D every trial is rejected and DE freezes at its start — E-197 adds an **adaptive crossover** capping mutated coordinates at ~15, and DE descends again (1240→882 at 100-D); PSO/DE stall counters also gain dimension-scaled patience. Existing optimize (39)+psoopt (6)+deopt (7)+saopt (7) unchanged. 4 checks | ngspice | [doc](enhancements_doc/Enhancement-197.md) | [opt100](examples/opt100_examples/) |
| 198 | ngspice **`stb` stability / loop-gain analysis** — the one-command loop-gain measurement (Spectre's `stb` / Middlebrook–Tian) ngspice lacked. Mark a feedback-loop break with a probe pair (`Vstb A B` series 0 V + `Istb 0 B` shunt 0 A, both bias-transparent); `stb Vstb Istb <ac-sweep>` runs a **voltage** and a **current** AC injection and combines them, `T = (Tv·Ti−1)/(Tv+Ti+2)`, so the result is load-corrected and **independent of the probe location** — which a single injection is not. Reports **phase margin** (180°+∠T at \|T\|=1) and **gain margin** (−\|T\|dB at ∠T=−180°) and stores complex `loopgain`. Front-end command (`com_stb.c`), reuses AC so it works under either solver. Verified vs closed-form op-amp margins; loaded-break == clean-break margins; reduced-PM design tracked. 5 checks | ngspice | [doc](enhancements_doc/Enhancement-198.md) | [stb](examples/stb_examples/) |
| 199 | **N-port Touchstone device** (`snp2va.py`) — drop a measured/simulated S-parameter block (filter, cable, package, amplifier) from a Touchstone `.sNp` file into a sim. Converts `.sNp` → a Verilog-A n-port realized with **`laplace_nd`**, so via the OSDI laplace machinery it works in **AC *and* transient** (no convolution engine). Pipeline: parse → S(f) → Y(f) → **common-pole vector fit** (Gustavsen) → `I(p_i) <+ Σ_j laplace_nd(V(p_j),num,den) + e_ij·ddt(V(p_j))` (the improper `e·s`/shunt-C term split out as `ddt`). **Automatic order selection** — climbs to capture high-order/delay-dominated responses, but stops at the knee once the error nears a floor, so noisy data is fit at its noise floor not over-fitted; stability enforced (RHP poles reflected → BIBO-stable); passivity checked+reported. Pure Python **stdlib** (no numpy — Householder-QR lstsq, complex matrix inverse, Durand-Kerner roots included). Any port count; S/Y/Z; MA/DB/RI. Verified: device matches the original R-L-C resonator in AC (5e−6) and transient (5e−3); a 5-pole LC ladder (order selection scales); a 3-port matches both coupled outputs (4e−9); noisy fit stays bounded. Stress-tested to near machine precision on resonators/notches/high-Q/ladders; delay-dominated lines degrade gracefully. 6 checks | openvaf‑r + ngspice | [doc](enhancements_doc/Enhancement-199.md) | [nport](examples/nport_examples/) |
| 212 | **Crash hardening: seven user-triggerable crashes** — static analysis + **argument fuzzing** of every frontend command and malformed/degenerate netlists found **seven reproducible SIGSEGV/SIGABRT crashes** in stock ngspice, all in *user-input handling* (never the numerical core, which was audited clean). Each was a missing edge-guard. **(1)** `iplot -w`/`-d` as the trailing token derefed NULL after the loop already proved `wl` could be NULL (`breakp.c`). **(2)** bare `altermod` — `com_alter` guards `if(!wl)` but `com_altermod` didn't, so an empty list reached `com_alter_common` → `wl_nthelem(NULL)` → deref (`device.c`). **(3)** `meas … FIND`/`WHEN` with a missing operand left `meas->m_vec` NULL → `vec_get(NULL)` → `strchr(NULL,…)`; fixed by making `vec_get` NULL-safe, the common sink (`vectors.c`). **(4)** an empty / all-commented deck + `.op` made an op plot with no vectors → `assert(pl_dvecs!=NULL)` **SIGABRT** (segfault under `-DNDEBUG`); guarded in `dotcards.c`. **(5)** KLU pole-zero setup printed a `bsearch`-miss diagnostic then **dereferenced the NULL anyway** — a broken guard (latent; `SMPmakeElt` keeps it unreachable) fixed to match `cktsetup.c` (`cktpzset.c`). **(6)** a self/circular **`.include`** recursed to stack overflow — `call_depth` was tracked but never limited; capped at 50 (`inpcom.c`). **(7)** a recursive **`.func f(x)={f(x)}`** expanded forever; capped the macro-expansion depth at 100 (`inpcom.c`). Netlist-recursion caps mirror OpenVAF's [E-148](enhancements_doc/Enhancement-148.md) hardening. New suite drives every repro (asserts graceful exit, not a signal) and confirms valid forms unchanged. 17 checks | ngspice | [doc](enhancements_doc/Enhancement-212.md) | [crashfix](examples/crashfix_examples/) |
| 211 | **Code-analysis bug fixes (XSPICE DC-op `else` + LTSPICE table free)** — a static-analysis pass (clang analyzer over every project-modified file, each finding confirmed by reading the code) surfaced two real defects. **(1)** `DCop()` (`dcop.c`) picks between the event-driven XSPICE solver and the analog solver with a **braceless `else`** whose body was originally `converged = CKTop(...)`. [E-188](enhancements_doc/Enhancement-188.md) (DC warm-start) inserted code between the `else` and `CKTop`, so the `else` then covered only its first statement and **`CKTop` ran unconditionally** — for a mixed-signal deck (A-devices) BOTH `EVTop` and `CKTop` ran, `CKTop` overwriting the event-driven DC result, and `wsize` was read **uninitialised** on the EVTop path. Fixed by hoisting `wsize` before the branch and bracing the else (the `#ifdef XSPICE` braces vanish together when XSPICE is off). **(2)** `inp_compat()` (`inpcom.c`) converts an LTSPICE `E … table=(…)` source with an **uninitialised** `char *ckt_array[100]`; a single-pair table (`ipairs==1`) then `tfree(ckt_array[2])` — a free of a garbage stack pointer. Fixed by zero-initialising the array (`tfree(NULL)` is a safe no-op). New suite guards both paths (mixed-signal DC op → 0.5 V; single-pair `table=(0.5,3)` → 3 V; plain analog DC op unchanged). 5 checks | ngspice | [doc](enhancements_doc/Enhancement-211.md) | [codeanalysis](examples/codeanalysis_examples/) |
| 210 | **PSS polish: `.pss` dot-card runs in batch + complex spectrum vectors** — two usability fixes to the [E-117](enhancements_doc/Enhancement-117.md) shooting-method periodic steady state, in the spirit of E-209. (1) A top-level **`.pss`** card by itself silently did nothing in batch (`ngspice -b` reported "no simulations run"), unlike `.tran`/`.ac`/`.hb`; it's now dispatched to the `pss` command via the deck→control mechanism ([E-146](enhancements_doc/Enhancement-146.md)/[E-162](enhancements_doc/Enhancement-162.md) pattern, in `inp.c`), so it auto-runs straight from the deck (decks that wrap it in `.control … run` keep working). (2) The frequency-domain plot `pss2` stored **magnitude only** — the DFT computed each harmonic's phase and discarded it, so `vp(out)` was unavailable, inconsistent with AC and with hb's E-209 vectors. `pss2` now publishes **complex** node vectors (`mag·e^{jφ}` via `OUTpData` instead of the real-only `CKTdump`): `mag(out)` reproduces the old magnitude spectrum exactly and `vp(out)` now recovers the phase; `pss1` (time-domain waveform) is unchanged. Backward-compat: complex `pss2` means `print out` emits `re,im` like AC (use `mag(out)`); the bundled `rc_pss.cir` was migrated `print b`→`print mag(b)`. Numerical PSS core untouched. Verified: `.pss` auto-runs in batch, and the spectrum resolves as complex (vm + vp) on a driven diode's harmonics; all existing PSS/PAC/PXF/solver-parity checks unchanged. 2 checks | ngspice | [doc](enhancements_doc/Enhancement-210.md) | [rfpss](examples/rfpss_examples/) |
| 209 | **`hb` publishes its spectrum as nutmeg vectors** — the [E-134](enhancements_doc/Enhancement-134.md) harmonic-balance command used to only *print* a table; the converged Fourier coefficients were freed before the user could touch them, so plotting an HB spectrum meant scraping stdout. Now a converged `hb <f0> <K>` leaves a fresh **`hb` plot** current, holding a real **`hbfrequency`** scale (`0, f0, …, K·f0`) and one **complex vector per node** (single-sided amplitude, so `mag(out)`/`vp(out)` match the table's `|V|`/phase columns) — so `plot mag(out)`, `print vdb(out)`, `wrdata spec mag(out) vp(out)` all work directly, and every accessor (`v(out)`, `vm`, `vdb`, `vp`, `real`, …) resolves. `HBanalyze()` gains an optional `struct hbspectrum *out` that hands the `Vr/Vi` arrays back instead of freeing them; `com_hb()`'s new `hb_publish()` builds the plot with the frontend vector API (`plot_alloc`/`dvec_alloc`/`vec_new`, the `stb`/`eye` idiom) — published from the frontend so no analysis `JOB` is needed (a bare `hb` in `.control` publishes cleanly). The printed table is kept (backward-compatible); the `.hb` dot-card publishes the same vectors. Verified: after `hb 100meg 5`, the published `mag(n)` equals the table `|V|` bit-for-bit across all six harmonics. 1 check | ngspice | [doc](enhancements_doc/Enhancement-209.md) | [hb](examples/hb_examples/) |
| 208 | **`pyplot -eye`: one-line matplotlib eye diagrams** — a convenience bridge between the [E-207](enhancements_doc/Enhancement-207.md) `eye` analysis and the [E-94](enhancements_doc/Enhancement-94.md) matplotlib back-end. `pyplot [name] -eye <expr> -ui <T> [eye opts]` runs the eye analysis *and* renders the result in one command. The raw `plot eye_wave vs eye_t` draws a single connected line through the folded samples (a scribble); `pyplot -eye` instead draws them as a **2-D histogram** (`hist2d`, log `LogNorm`, `turbo`, empty cells masked) so overlapping traces accumulate into a **persistence eye** — exactly how a sampling scope paints one — annotated with the threshold, the eye-centre sampling instant, an eye-**height** double-arrow, an eye-**width** double-arrow, and a title carrying UI / height / width (% UI) / RMS jitter. `com_pyplot()` detects `-eye`, runs `com_eye()` on the trailing tokens, then the new `ft_pyplot_eye()` reads back the folded vectors + metrics and writes the `.data`/`.py`, launched via the same mechanism as `ft_pyplot()` — so it honours every `pyplot_*` setting (`pyplot_terminal=png/svg/pdf` headless vs an interactive window, `pyplot_python`/`backend`/`figsize`/`style`, a `dark` sheet flipping the annotation colours). No numerical-core changes; the eye math is E-207's. Base name defaults to `eye`. 4 checks | ngspice | [doc](enhancements_doc/Enhancement-208.md) | [pyplot](examples/pyplot_examples/) |
| 207 | **eye diagram / jitter analysis** (`eye`) — the core signal-integrity measurement for serial links / SerDes, which ngspice lacked. `eye <expr> -ui <T>` folds a received data waveform modulo the unit interval and reports the standard metrics: it auto-detects the two logic rails + decision threshold, finds every threshold crossing, estimates the UI phase and each crossing's **TIE** (time-interval error) → **jitter RMS/pp**, measures the **eye height** (vertical opening at the sampling instant) and **eye width** (UI − jitter_pp, plus the width at **BER 1e-12** via the Gaussian-RJ tail), and folds the waveform into `eye_wave` vs `eye_t` whose scatter plot *is* the eye diagram (`plot eye_wave vs eye_t`). A self-contained front-end command (`com_eye.c`, in the `fft`/`linearize` family), solver-independent, publishing 11 result vectors. Verified on signals with known metrics: an ideal clock → ~0 jitter + full-UI eye; a clock with **known injected Gaussian jitter** (σ=20 ps) → recovered to <1% (18.6 ps rms, 99 ps pp), with eye width = UI − jitter_pp. 6 checks | ngspice | [doc](enhancements_doc/Enhancement-207.md) | [eye](examples/eye_examples/) |
| 206 | **design centering / yield optimization** (`optimize -center`) — the capstone that **fuses two whole subsystems**: the optimizer (nm/lm/pso/de/sa, E-130–197) and the Monte-Carlo yield suite (`agauss`/`mccorr` + `montecarlo` specs, E-149–151). The outer optimizer searches the **nominal design point** (`-dparam` knobs feeding the `agauss` centres); at each candidate the inner loop runs `-samples N` Monte-Carlo draws (each `reset` re-samples the deck's process variation around the centre), scores every `-spec`, and reduces to the **worst-case Cpk** (the smooth objective, maximized) and the pass-fraction **yield** (reported). Cpk beats raw yield as the objective because yield is a granular step function that stalls a simplex, while Cpk is continuous and monotone in yield; a fixed inner seed gives every candidate the same draws (common random numbers), and with `-lhs` the stratified mean ≈ 0 so the optimum lands on the analytic centre. `com_optimize.c` only (`opt_eval_center`); `dcenter_yield`/`dcenter_cpk` result vectors. Verified on synthetic problems with known optima: output ~ N(xc, 0.5), spec [4,6] → centres xc from 4.0 to **5.00** (Cpk 0.667, yield 50%→96%); two knobs centre independently. 5 checks | ngspice | [doc](enhancements_doc/Enhancement-206.md) | [dcenter](examples/dcenter_examples/) |
| 205 | **`pre_snp` low-rank residue factorization** — the E-200/201 N-port Touchstone device emitted an **O(N²)-term, O(N·Np)-filter** shared-pole model, so OpenVAF's compile time grew super-linearly and capped the practical port count at ~20–24 (an N=32 block compiles in ~80 s — the vector fit scales to N=100, the emitted model's compile does not). When ports couple through a few shared modes (multi-port filters/cavities/packages), each pole's N×N residue matrix has rank **r ≪ N**. `pre_snp` now SVD-factors each channel (a new in-C Jacobi SVD) and — since `laplace` is linear, `Σⱼ V[j][m]·laplace(V(pⱼ)) = laplace(Σⱼ V[j][m]·V(pⱼ))` — filters the **r combined inputs once** instead of all N, collapsing filters **O(N·Np)→O(r·Np)** and coupling **O(N²)→O(N·r)**. A 3-mode block: N=32 compile **81 s→11.6 s** (192→8 filters); response exact (AC 4e-7, tran 3e-5). Full-rank blocks (a ladder) keep the dense form **bit-identically** (auto-detect leaves them alone); `PRE_SNP_DENSE` forces dense. 5 checks | ngspice + openvaf‑r | [doc](enhancements_doc/Enhancement-205.md) | [lowrank](examples/lowrank_examples/) |
| 204 | ngspice **auto-triggering DC convergence** (`.option convhelp`) — ngspice's operating-point solver already auto-escalates through gmin- and source-stepping, but the two strongest aids — the globalized damped-Newton **line search** (E-111) and **pseudo-transient continuation** (E-127) — only fired if the user hand-set `.option linesearch`/`ptcont`, and nothing said which aid rescued a run. `convhelp` makes the whole cascade one automatic ladder (Newton+line-search → gmin → source → pseudo-transient → optran) and reports the aid that converged. Line search reduces to a plain full Newton step on easy points, so the default (off) path is byte-identical; `errpreset=conservative` enables it, an explicit `convhelp=0` overrides. On a stiff behavioral-exp deck where plain Newton lands on a spurious 70.5 V root, `convhelp` auto-fires pseudo-transient continuation to the **correct 0.8379 V** with no `.option ptcont`. Result-neutral + silent on a normal-circuit battery; both solvers. 35 checks | ngspice | [doc](enhancements_doc/Enhancement-204.md) | [convhelp](examples/convhelp_examples/) |
| 203 | **`.meas ac` gain / phase margin (+ batch `vdb`/`vp` fix)** — an audit found ngspice's `.measure` already covers TRIG/TARG (VAL/RISE/FALL/CROSS/LAST/TD/AT), FIND…WHEN, AVG/RMS/INTEG/DERIV, MIN/MAX/PP, ERR, `param=`; the one gap was **AC margins**. The manual recipe `FIND vdb WHEN vp=-180` **cannot work** — `vp()` is wrapped to (−180,180] so the phase never equals −180°. Added first-class **`phase_margin`** (180 + phase at the 0 dB gain crossover) and **`gain_margin`** (−gain(dB) at the −180° phase crossover), computed on the **unwrapped** phase, each reporting its crossover freq; a loop with no finite −180° crossover (infinite GM) is reported as such. Verified to closed form (2-pole PM 84.89°; 3-pole PM −42.75°, GM −15.92 dB — exact). Also fixed **`gettok_iv`** truncating `vdb(out)`→`vd` (broke the batch measure auto-save → "can't parse 'vd'" / "no data saved for AC"). 5 checks | ngspice | [doc](enhancements_doc/Enhancement-203.md) | [acmargin](examples/acmargin_examples/) |
| 202 | **`.sp` S-parameter scalability — O(N!)→O(N³) matrix inverse** — producing an N-port Touchstone file with ngspice's own `.sp` analysis was pathologically slow (8-port ~13s, **10-port ~18min**, growing ~×N per added port). Profiling put ~99% of the time in `CKTspCalcSMatrix`, which inverts the N×N port matrix at every frequency (for S/Y/Z). The complex inverse (`cinverse`, `maths/dense/dense.c`) used the **adjugate/determinant** method (Cramer's rule) — and `cdet` is a **recursive cofactor expansion, O(N!)**, with the adjugate doing N² of them → `O(N·N!)` (exactly the ×7/×8/×9-per-port `6!→7!→8!` blowup). Replaced `cinverse`/`cinversedest` with **Gauss-Jordan elimination + partial pivoting** — `O(N³)`, drop-in, same result. `.sp` at N=8 **13s→0.3s**, N=10 **18min→0.02s** (~50,000×), N=32 0.09s; extracted S-params unchanged (verified vs closed form to 2e−7). Same inverse speeds the `.psp`/PSS path (`dcpss.c`). With [E-201](enhancements_doc/Enhancement-201.md) the whole extract→convert→simulate workflow now scales. 2 checks | ngspice | [doc](enhancements_doc/Enhancement-202.md) | [spscale](examples/spscale_examples/) |
| 201 | **`pre_snp` scalability** — the E-200 converter struggled from ~8 ports up (an 8-port took ~190s; its pole solve stacked all N² elements into one dense least-squares → **O(N⁴) memory, O(N⁶) compute**, ~8TB at N=100; the emitted model gave every element its own `laplace_nd` bank → O(N²·Np) state). Four fixes rebuild it: **(1) Fast Vector Fitting** — the block-arrow pole-ID is solved by Householder-reducing each element block independently and stacking only the shared-pole coupling → mem **O(N⁴)→O(N²)**, compute **O(N⁶)→O(N²·Ns·Np²)**; **(2) reciprocity** — symmetric Y ⇒ fit the upper triangle + mirror (~2×); **(3) iteration cap** — stop relocating once poles settle (was fixed 12); **(4) shared-pole realization** — filter each input port **once** (real pole → `V/(s−p)`, conj pair → real `(s−σ)/D`+`ω/D` bases, residue = a real weight), so the model is **O(N·Np)** `laplace_nd` not O(N²·Np). Also fixed 3 latent bugs the stress test exposed: an order-selection **double-free** when poles lose conjugate-pair adjacency (→`canon_poles`), a **256-entry stack array** (`pv[16*16]`) that hard-capped ports at 16 (→heap), and an **OpenVAF crash** on integer-looking laplace coefficients (`'{1}`→`'{1.0}`). Fit at N=8 **220s→3.6s**; N=64 in 47s; **N=100 in 2.75min** at 9.6e−4. DC/AC/transient still match (N=16 to 1.3%); new limit is OpenVAF compile time (~N=20–24 end-to-end). 9 checks | ngspice + openvaf‑r | [doc](enhancements_doc/Enhancement-201.md) | [presnp](examples/presnp_examples/) |
| 200 | **built-in `pre_snp` command** — folds the E-199 Touchstone→Verilog-A converter into ngspice itself as a C command (no external script, no Python). Inside `.control`, `pre_snp <file.sNp> [module]` parses the `.sNp`, common-pole **vector-fits** it, writes the `.va`, and invokes **`openvaf-r`** to compile the `.osdi` — a one-line analog of `pre_osdi`. Identical numerics to `snp2va.py`, reimplemented in **C** (`double _Complex`: Householder-QR lstsq, complex matrix inverse, Durand-Kerner roots, automatic order selection, RHP-pole reflection). Like `pre_osdi` it runs **before circuit parsing**, and on top of that **every `pre_snp` is forced to run before every other `pre_` command** (two-pass pre-command execution), so the `.osdi` it generates already exists when `pre_osdi` loads it — even with the `pre_osdi` line written first. Finds the compiler via the `openvaf` variable → `OPENVAF` env → `$SPICE_LIB_DIR/openvaf-r` → `PATH`. Verified: `pre_snp` writes+compiles inside ngspice; the device matches an R-L-C resonator in **AC** (5e−6) and **transient** (5e−3); a **3-port** star matches both coupled outputs (4e−9); and with `pre_osdi` written before `pre_snp` the model still loads (ordering guarantee). 5 checks | ngspice + openvaf‑r | [doc](enhancements_doc/Enhancement-200.md) | [presnp](examples/presnp_examples/) |
