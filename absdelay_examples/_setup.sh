# Shared setup for the absdelay example/benchmark scripts. Sourced, not run.
#
# Selects the right prebuilt binaries from bin/<os>/<arch>/ for the current
# machine and (re)compiles the absdelay model for this platform, because a
...
mkdir -p "$_BUILD_DIR"
OSDI="$_BUILD_DIR/absdelay.osdi"
if ! ( cd "$_SETUP_DIR" && "$VAF" absdelay.va -o "$OSDI" ) >/dev/null 2>&1; then
  echo "openvaf-r failed to compile absdelay.va" >&2
  exit 1
fi
[ -f "$OSDI" ] || { echo "absdelay.osdi was not produced at $OSDI" >&2; exit 1; }
