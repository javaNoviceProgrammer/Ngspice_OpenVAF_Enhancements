        ...
        struct dvec *vecs)
{
    ...
    /* `set pyplot_terminal png` -> render headless (Agg) to <file>.png rather
       than opening an interactive window. */
    ...
    for (v = vecs; v; v = v->v_link2) {
        ...
        if (boxes)
            ...
            fprintf(file, "plot(d[:, %d], d[:, %d], ", col, col + 1);
        fprintf(file, "label=");
    ...
    if (to_png) {
        ...
        fprintf(file, " + '.png', dpi=100)\n");
        fprintf(file, "print('pyplot: wrote %s.png')\n", filename);
    } else {
