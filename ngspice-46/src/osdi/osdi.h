 ...
 *
 * This file started as header.lua output for the published OSDI 0.3
 * specification, but the in-repo ABI has since diverged deliberately
 * (Enhancement-45 node nodeset field, E-51 ac_stim descriptor tail, E-54
 * paired load_noise densities, E-523 absdelay flags, ...). It describes the
 * openvaf-reloaded layout this ngspice build actually reads -- version 0.7+
 * as negotiated in osdiregistry.c; plain v0.3 objects are rejected at load
 * (OSDI-layer audit).
 *
 */
...
/* OSDI-layer audit: the v0.3 "CURR" constants that gated the (broken)
 * original-OpenVAF load path are gone -- version negotiation lives in
 * osdiregistry.c and requires the openvaf-reloaded layout (>= 0.7). */
