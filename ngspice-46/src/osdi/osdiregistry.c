...
extern OsdiObjectFile load_object_file(const char *input) {
  ...
  if (OSDI_DESCRIPTOR_SIZE) {
    ...
    // Must be version>=0.7: Enhancement-54 changed load_noise to fill
    // [flat, j*omega] power pairs (stride 2). Enhancement-45 changed the
    // OsdiNode stride and
    ...
    if (!(
      (OSDI_VERSION_MAJOR == 0 && OSDI_VERSION_MINOR >= 7) ||
      OSDI_VERSION_MAJOR >= 1
    )) {
      printf("NGSPICE supports OpenVAF-reloaded OSDI version >= 0.7 but \"%s\" uses v%d.%d!\n"
  ...
  for (uint32_t i = 0; i < OSDI_NUM_DESCRIPTORS; i++) {
    ...
    /* Enhancement-401 */
    uint32_t n_term_shorts = 0;
    const void *term_shorts_ptr = NULL;
    if (term_short_counts) {
      n_term_shorts = term_short_counts[i];
      if (n_term_shorts > 0 && term_short_infos_base) {
        term_shorts_ptr = (const char *)term_short_infos_base +
                          term_short_info_offset * term_short_info_size;
        term_short_info_offset += n_term_shorts;
      }
    }
