...
""",
    ...
    "block_091_1": dict(fp="module adc", cls="ok",
        note='8-bit ADC: param-dependent bus width folds (E-91) and the analog-block genvar for-loop is unrolled at elaboration (E-407)'),
    ...
    "block_117_1": dict(fp="module dac", cls="ok",
        note="DAC: the genvar loop is unrolled (E-407); the LRM's own hand-unrolled `dac8` sits beside it in this file and gives the identical output"),
...
""",
    ...
    "block_134_1": dict(fp="genvarexp", cls="ok",
        note='genvar expression indexing a vectored net; unrolled at elaboration (E-407)'),
...
""",
    ...
    "block_169_1": dict(fp="rcline", cls="limitation",
        expect="generate for: loop bound must be a compile-time-constant integer",
        note="bare module-level generate-for now parses (E-96); still blocked on the parameter loop bound N (generate structure cannot depend on a runtime-bindable parameter, E-67 scope) + undefined resistor/capacitor primitives"),
    "block_169_2": dict(fp="rcline2", cls="limitation",
        expect="generate for: loop bound must be a compile-time-constant integer",
        note="bare module-level generate-for now parses (E-96); still blocked on the parameter loop bound N (E-67 scope) + undefined primitives"),
    ...
    "block_172_1": dict(fp="genblk2", cls="limitation",
        expect="generate if: the condition",
        note="bare module-level generate-if now parses (E-96); still blocked on the parameter/implicit-genblk condition (generate structure cannot depend on a parameter, E-67 scope)"),
