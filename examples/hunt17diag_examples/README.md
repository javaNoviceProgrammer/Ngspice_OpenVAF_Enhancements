...
escaped identifier exported with a `+`, the depth-bound cascade on a flat
sum (the 999-term sum itself legal since [E-718](../../enhancements_doc/Enhancement-718.md),
which raised the bound from 1000 to 32 768 levels; the cascade check uses a chain
past it), and `absdelay`/`$bound_step` with a deck-fixed negative argument. Two
...
Run: `python3 verify_hunt17diag.py` (24 checks per solver, both solvers).
