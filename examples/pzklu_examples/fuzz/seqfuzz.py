...
OSDI = os.path.join(HERE, "lin.osdi")
...
ANALYSES = [
    ...
    # widened pool: the RF/steady-state and workflow commands, which reuse the
    # PSS/S-parameter machinery and so share its per-analysis state
    "rfstab", "stb V1 V2 dec 5 1e5 1e8", "envelope mid 1meg 20u",
    "linearize v(mid)", "sweep V1 0 1 0.5 v(mid)",
    "wcd v(mid) 0.3", "qpss v1#branch 1e6 1.3e6 hb 3 3",
]
...
MUTATORS = [
    ...
    # SOLVER SWITCHING mid-session. KLU and SPARSE keep SEPARATE per-instance
    # matrix-pointer sets (osdidefs.h: the *_csc / *_cx pairs are NULL under
    # SPARSE), so flipping between them between analyses is the same shape of
    # hazard as E-365: state established for one backend, used by the other.
    "option klu", "option sparse",
    "option gmin=1e-10", "option reltol=1e-4", "option noopiter",
    "altermod mylin r = 900",
]
