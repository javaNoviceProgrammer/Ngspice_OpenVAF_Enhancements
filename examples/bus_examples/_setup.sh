# Shared setup for the bus_examples scripts. Sourced, not run.
#
# Selects the right prebuilt binaries from bin/<os>/<arch>/ for the current
# machine and (re)compiles the bus_buffer model for this platform, because a
...
#   OSDI  absolute path to a freshly compiled bus_buffer.osdi (in .build/)

# Directory containing this file (= bus_examples/)
...
mkdir -p "$_BUILD_DIR"
OSDI="$_BUILD_DIR/bus_buffer.osdi"
if ! ( cd "$_SETUP_DIR" && "$VAF" bus_buffer.va -o "$OSDI" ) >/dev/null 2>&1; then
  echo "openvaf-r failed to compile bus_buffer.va" >&2
  exit 1
fi
[ -f "$OSDI" ] || { echo "bus_buffer.osdi was not produced at $OSDI" >&2; exit 1; }
