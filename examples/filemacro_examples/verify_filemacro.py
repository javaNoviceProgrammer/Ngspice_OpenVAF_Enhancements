...
`define body. Checks:
  ...
  [4] `__LINE__ inside a `define body reports the line of USE (LRM 10.7,
      round-4 audit -- the pre-pass pinned the definition site here before)
"""
import os
import re
import subprocess
...
use_line = next(i for i, l in enumerate(src, 1) if l.strip() == "`WHERE;")
...
check(f"`define-body `__LINE__ == {use_line} (line of use, LRM 10.7)",
      m_macro and int(m_macro.group(2)) == use_line, m_macro.group(2) if m_macro else "-")
