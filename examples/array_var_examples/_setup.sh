# Shared setup for the array_var_examples scripts. Sourced, not run.
#
# Selects the right prebuilt binaries from bin/<os>/<arch>/ for the current
# machine and (re)compiles the array_var_fir model for this platform, because
# a .osdi is a native shared library and is therefore architecture-specific.
#
...
#   OSDI  absolute path to a freshly compiled array_var_fir.osdi (in .build/)

# Directory containing this file (= array_var_examples/)
...
mkdir -p "$_BUILD_DIR"
OSDI="$_BUILD_DIR/array_var_fir.osdi"
if ! ( cd "$_SETUP_DIR" && "$VAF" array_var_fir.va -o "$OSDI" ) >/dev/null 2>&1; then
  echo "openvaf-r failed to compile array_var_fir.va" >&2
  exit 1
fi
[ -f "$OSDI" ] || { echo "array_var_fir.osdi was not produced at $OSDI" >&2; exit 1; }
