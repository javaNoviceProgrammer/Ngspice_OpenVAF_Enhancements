# Shared setup for the bessel_filter_examples scripts. Sourced, not run.
#
# Selects the right prebuilt binaries from bin/<os>/<arch>/ for the current
# machine and (re)compiles the bessel5 model for this platform, because a
...
#   OSDI  absolute path to a freshly compiled bessel5.osdi (in .build/)

# Directory containing this file (= bessel_filter_examples/)
...
mkdir -p "$_BUILD_DIR"
OSDI="$_BUILD_DIR/bessel5.osdi"
if ! ( cd "$_SETUP_DIR" && "$VAF" bessel5.va -o "$OSDI" ) >/dev/null 2>&1; then
  echo "openvaf-r failed to compile bessel5.va" >&2
  exit 1
fi
[ -f "$OSDI" ] || { echo "bessel5.osdi was not produced at $OSDI" >&2; exit 1; }
